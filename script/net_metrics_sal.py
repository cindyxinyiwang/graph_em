...
def network_properties(orig, net_mets, synth_graphs_lst, name='', out_tsv=False):
		...
		if 'gcd' in net_mets:
			...
			a=1.0*np.array(gcd_hrg)
			confidence=0.95
			gcd_h = scipy.stats.sem(a)*scipy.stats.t._ppf((1+confidence)/2, len(a)-1)
			print
			...
			print "name", "mean", "std", "0.95 significance"
			print "{}, {}, {}, {}".format(name,gcd_hrg_mean,gcd_hrg_std, gcd_h)
			for d in gcd_hrg:
				print d
